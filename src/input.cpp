...
void AxisManager::update_axis_values(float delta_time) {
    ...
    for (auto& mapping : mappings) {
        ...
        if (auto it =
            ...
            if (is_gamepad_axis(mapping.key)) {
                ...
                // #TODO: Snapping
                float delta = glm::abs(mapping.sensitivity) * delta_time;
...
GLFWwindow* InputEventManager::win;

void InputEventManager::init(GLFWwindow* window) {
    // register callbacks
    glfwSetKeyCallback(window, &InputEventManager::keyboard_callback);
    glfwSetCursorPosCallback(window,&InputEventManager::mouse_position_callback);
    glfwSetMouseButtonCallback(window, &InputEventManager::mouse_button_callback);
    glfwSetScrollCallback(window, &InputEventManager::scroll_callback);
...
void InputEventManager::mouse_position_callback(GLFWwindow* win,
                                                double xpos,
                                                double ypos) { 
    struct first_frame_window_size {
        int w, h;
        first_frame_window_size(GLFWwindow* win) {
            glfwGetWindowSize(win, &w, &h);
        }
    };

    static first_frame_window_size winsize(win);

    static float last_x = winsize.w / 2.0f;
    static float last_y = winsize.h / 2.0f;
...
void InputEventManager::process_events(float delta_time) {
    process_keyboard_events();
    process_mouse_events();
    JoystickInputManager::update_key_data();
    AxisManager::update_axis_values(delta_time);
}
